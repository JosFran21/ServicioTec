<?php
    ...
    class logData
    {
        ...
        {   
            if(sizeof(self::$data)==0)
            {
                ...
                if(isset($_COOKIE['sessionKey']))
                {
                    ...
                    self::$data = dbConnection::select(["*"], "users", [["sessionKey", $_COOKIE['sessionKey']]]);
                    if(sizeof(dbConnection::select(["*"], "mortals", [["id_usuario", self::$data[0]['id']]]))>0)
                    {
                        self::$type = "User";
                    }
                    elseif(sizeof(dbConnection::select(["*"], "superusers", [["id_usuario", self::$data[0]['id']]]))>0)
                    {
                        self::$type = "SUser";
                    }
                    return true;
        ...
        public static function logIn($name, $pass, $stay = false)
        {
            ...
            $obtainedData = dbConnection::select(["*"], "users", [["email", $name],["password", $cipher_pass]]);
            ...
            if(sizeof(self::$data)!=0)
            {
                ...
                if($stay)
                {
                    ...
                    dbConnection::update("users", ["sessionKey"], [$cipherKey], [["id", self::$data[0]['id']]]);
        ...
        public static function logOut()
        {
            ...
            dbConnection::update("users", ["sessionKey"], [""], [["id", self::$data[0]['id']]]);
